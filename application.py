...
@app.route("/")
def index():
    products = db.execute("SELECT * FROM products ORDER BY type ASC")
    ...
    if 'user' in session:
        shoppingCart = db.execute("SELECT type, image, SUM(qty), SUM(subTotal), price, id FROM cart GROUP BY type")
        shopLen = len(shoppingCart)
...
def buy():
    ...
    if session:
        ...
        db.execute("INSERT INTO cart (id, qty, type, image, price, subTotal) VALUES (:id, :qty, :type, :image, :price, :subTotal)", id=id, qty=qty, type=type, image=image, price=price, subTotal=subTotal)
        shoppingCart = db.execute("SELECT type, image, SUM(qty), SUM(subTotal), price, id FROM cart GROUP BY type")
        shopLen = len(shoppingCart)
...
def filter():
    if request.args.get('category'):
        ...
        products = db.execute("SELECT * FROM products WHERE category = :query ORDER BY type ASC", query=query )
    if request.args.get('sale'):
        query = request.args.get('sale')
        products = db.execute("SELECT * FROM products WHERE onSale = :query ORDER BY type ASC", query=query)
    if request.args.get('id'):
        query = int(request.args.get('id'))
        products = db.execute("SELECT * FROM products WHERE id = :query ORDER BY type ASC", query=query)
...
def checkout():
    ...
    for item in order:
        db.execute("INSERT INTO purchases (uid, id, type, image, quantity) VALUES(:uid, :id, :type, :image, :quantity)", uid=session["uid"], id=item["id"], type=item["type"], image=item["image"], quantity=item["qty"] )
    # Clear shopping cart
...
def remove():
    # Get the id of product selected to be removed
